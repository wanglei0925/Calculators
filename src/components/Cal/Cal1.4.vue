<template>
    <div class="calculators_right_wrapper ">
        ...
        <div class="cal_form border-ra cal_fff_small cal_mar_bot_3">
            等额本金还款计算器（每月偿还本金相同，月还款总额逐月递减），将帮助用户计算在不同的借款期限、不同的贷款利息下，采用等额本息还款方式时，需要支付的月供、利息总额和还款总额。</div>
    ...
    edu() {
      ...
      let lilv =
        this.optionStyle == 1
          ...
          : this.mj.lilv / 100 / 12;
      //等额本金  每月递减的金额
      let mydj = cal.jsCa(allMoney, monthNum, lilv);

      this.allMoney = allMoney.toFixed(2);
      this.monthNum = monthNum;
      this.debj.firstMoney = (
        this.allMoney / this.monthNum +
        mydj * this.monthNum
      ).toFixed(2);
      this.debj.monthDc = mydj.toFixed(2);
      var debjTotal = 0;
      for (let i = 1; i <= this.monthNum; i++) {
        debjTotal += this.allMoney / this.monthNum + mydj * i;
      }
      this.debj.allLixi = (debjTotal - this.allMoney).toFixed(2);
